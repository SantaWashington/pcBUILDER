...
public class SpawnObject_Create : MonoBehaviour
{

    private GameObject savedObjectPrefab;
    private GameObject savedObjectInstance;
    public GameObject SpawnPoint;
    private Rememberproc rememberproc;

    private void Start()
    {
        rememberproc = Rememberproc.Instance;
        if (rememberproc != null)
        {
            string savedObjectName = rememberproc.GetSavedObjectState();
            ...
            if (!string.IsNullOrEmpty(savedObjectName))
            {
                ...
                if (savedObjectPrefab != null)
                {
                    ...
                    // Сбрасываем сохраненное имя объекта в Rememberproc
                    rememberproc.ClearSavedObjectState();
                    Rememberproc.Instance.Reset();
                    GameObject[] objects2 = GameObject.FindGameObjectsWithTag("CPU");
