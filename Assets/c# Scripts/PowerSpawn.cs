...
public class PowerSpawn : MonoBehaviour
{
    ...
    private RememberPower rememberpower;

    private void Start()
    {
        rememberpower = RememberPower.Instance;
        if (rememberpower != null)
        {
            string savedObjectName = rememberpower.GetSavedObjectState();
            if (!string.IsNullOrEmpty(savedObjectName))
            {
                ...
                GameObject[] objects6 = GameObject.FindGameObjectsWithTag("CPU");
                ...
                if (savedObjectPrefab != null)
                {
                    ...
                    rememberpower.ClearSavedObjectState();
                    RememberPower.Instance.Reset();
                    GameObject[] objects2 = GameObject.FindGameObjectsWithTag("Power");
